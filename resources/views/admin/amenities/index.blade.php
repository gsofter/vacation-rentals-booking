               ...
               <div class="row">
                ...
                <div class="col-sm-12">
                    ...
                    <div class="card-header table-card-header text-right">                            
                        <a href="{{url('admin/add_amenity')}}" class="btn btn-primary">Add Amenities</a>
                    </div>
                    ...
                    @if(count($data) != 0)
                        ...
                        <tr>
                            <td>{{$row->id}}</td>                            
                            <td>{{$row->getTypeNameByTypeID()}}</td>                                                       
                            <td>{{$row->name}}</td>                                                       
                            <td>{{$row->description}}</td>                                                       
                            <td><img src="{{asset("images/amenities/$row->icon")}}" style="height:30px"></td>                                                    
                            <td>{{$row->status}}</td>                                                       
                            <td class="text-center">
                                <a href="{{url( 'admin/edit_amenity/' . $row->id )}}" class="btn btn-xs btn-primary" data-toggle="tooltip" title="" data-original-title="Edit">
         ...
         <div class="modal fade" id="default-Modal" tabindex="-1" role="dialog" style="z-index: 1050; display: none;" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form method="POST" action="{{url('admin/delete_amenity')}}" accept-charset="UTF-8" class="form-horizontal bordered" role="form">
                        {{ csrf_field() }}
