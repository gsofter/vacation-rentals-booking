               ...
               <div class="row">
                ...
                <div class="col-sm-12">
                    ...
                    <div class="card-header table-card-header text-right">                            
                        <a href="{{url('admin/add_coupon_code')}}" class="btn btn-primary">Add Coupon Code</a>
                    </div>
                    ...
                    <thead>
                        ...
                        <th>ID</th>
                        <th>Description</th>
                        <th>Stripe Coupon Code</th>
                        {{--  <th>Branitree Coupon Code</th>  --}}
                        <th>Amount</th>
                        <th>Type</th>
                        {{--  <th>Currency Code</th>  --}}
                    ...
                    @if(count($data) != 0)
                        ...
                        <tr>
                            ...
                            <td>{{$row->stripe_coupon_code}}</td>
                            {{--  <td>{{$row->braintree_coupon_code}}</td>  --}}
                            <td>{{$row->amount}}</td>
                            <td>{{$row->type}}</td>
                            {{--  <td>{{$row->currency_code}}</td>  --}}
                            {{--  <td>{{$row->max_redemptions}}</td>  --}}
                            <td>{{$row->duration}}</td>
                            <td>{{$row->expired_at}}</td>
                            <td>{{$row->status}}</td>
                            <td>
                                <a href="{{url( 'admin/edit_coupon_code/' . $row->id )}}" class="btn btn-xs btn-primary" data-toggle="tooltip" title="" data-original-title="Edit">
         ...
         <div class="modal fade" id="default-Modal" tabindex="-1" role="dialog" style="z-index: 1050; display: none;" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form method="POST" action="{{url('admin/delete_coupon_code')}}" accept-charset="UTF-8" class="form-horizontal bordered" role="form">
                        {{ csrf_field() }}
