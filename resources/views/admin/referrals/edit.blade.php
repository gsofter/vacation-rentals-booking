               ...
               <div class="row">
                ...
                <div class="col-sm-12">
                    ...
                    <div class="card-block">
                        ...
                        @yield('content')
                        <form method="POST" action="{{url('admin/edit_referral/' . $referral->id)}}" accept-charset="UTF-8" class="form-horizontal bordered" role="form">
                                {{ csrf_field() }}
                                        ...
                                        <select name="user_id" id="user_id" class="form-control required" required>
                                            ...
                                            @if(count($referrals) != 0)
                                                @foreach($referrals as $row)
                                                    <option value="{{$row->id}}" <?php echo $referral->user_id==$row->id?"selected":"";  ?>>{{$row->email}} - "ID" : {{$row->id}}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-6">
                                        <select name="friend_id" id="friend_id" class="form-control required" required>
                                            ...
                                            @if(count($referrals) != 0)
                                                @foreach($referrals as $row)
                                                <option value="{{$row->id}}" <?php echo $referral->friend_id==$row->id?"selected":"";  ?>>{{$row->email}} - "ID" : {{$row->id}}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-6">
                                    <input type="text"  name="credited_amount" value="{{$referral->credited_amount}}" class="form-control" placeholder="Credited Amount" required>
                                </div>
                            </div>
                            <div class="form-group row">
